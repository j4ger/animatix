...
impl OffscreenRenderer {
    ...
    pub fn render_timeline_with_debug(
        ...
        let scene = {
            ...
            let filter_backend = self.create_filter_backend(dimensions)?;
    ...
    /// Create a dedicated filter backend with its own GPU resources.
    fn create_filter_backend(&self, dimensions: SceneDimensions) -> Result<OffscreenFilterBackend, String> {
        let device = self.device.clone();
        let queue = self.queue.clone();
        let core = RendererCore::new(&device, &queue)
            .map_err(|e| format!("Failed to create filter renderer core: {e}"))?;
        Ok(OffscreenFilterBackend::new(device, queue, core, dimensions))
    }
...
/// GPU-backed filter backend that owns its own renderer core and temporary
/// targets. Created on-demand per frame by [`OffscreenRenderer`].
struct OffscreenFilterBackend {
    device: wgpu::Device,
    queue: wgpu::Queue,
    core: RendererCore,
    output_texture: Option<wgpu::Texture>,
...
impl OffscreenFilterBackend {
    fn new(device: wgpu::Device, queue: wgpu::Queue, core: RendererCore, dimensions: SceneDimensions) -> Self {
...
impl FilterBackend for OffscreenFilterBackend {
