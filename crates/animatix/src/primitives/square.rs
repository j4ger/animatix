...
impl Primitive for SquarePrimitive {
    fn type_name(&self) -> &'static str { "Square" }
    fn display_name(&self) -> &'static str { "Square" }
    fn category(&self) -> ActorCategory { ActorCategory::Shape }
    fn icon_id(&self) -> &'static str { "square" }
    ...
    fn render(&self, ctx: &RenderCtx) -> Option<Vec<VelloPath>> {
        let path = KurboShape::Rect {
            ...
            y0: -(ctx.state.size[0] as f64),
            x1: ctx.state.size[0] as f64,
            y1: ctx.state.size[0] as f64,
        }
        .to_path_default();
