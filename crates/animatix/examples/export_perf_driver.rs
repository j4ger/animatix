...
fn main() {
    ...
    // Queue one frame's GPU work without blocking (PF-7 pipelining).
    let queue_frame = |renderer: &mut OffscreenRenderer, t: f64| -> PendingFrame {
        if let Some(composition) = &composition {
            ...
            if let Some(blend) = transition_blend {
                ...
                return renderer
                    ...
                    .expect("queue transition frame");
            }
            let scene = &composition.scenes[&scene_name].timeline;
            return renderer
                .begin_frame_with_debug(scene, local_time_s, dims, Default::default())
                .expect("queue frame");
        }
        renderer
            .begin_frame_with_debug(&timelines[0], t.min(duration_s), dims, Default::default())
            .expect("queue frame")
    };

    // ── Settle: shader pipelines, caches, layout ──
    for i in 0..30u32 {
        let frame = queue_frame(&mut renderer, ((i % 30) as f64) / fps);
        std::hint::black_box(renderer.wait_frame(frame).expect("settle frame"));
    }
    ...
    // ── Measured window (pipelined: begin N+1 before waiting on N) ──
    ...
    let mut pending = Some(queue_frame(&mut renderer, 0.0));
    let loop_wall = std::time::Instant::now();
    for i in 1..=frames {
        let next = (i < frames)
            .then(|| queue_frame(&mut renderer, (i as f64 / fps) % duration_s.max(0.001)));
        // The wall clock covers ONLY the wait+copy of frame N — frame N+1's
        // GPU work was queued before the wait, overlapping with it.
    ...
    let loop_total = loop_wall.elapsed();
    ...
    println!(
        "  fps: {:>8.1}   wait ms: p50 {:>7.2}  p90 {:>7.2}  mean {:>7.2}",
        frames as f64 / loop_total.as_secs_f64(),
        p(0.50),
        p(0.90),
    ...
    let mean_wait = total.as_secs_f64() * 1000.0 / frames as f64;
    let mean_loop = loop_total.as_secs_f64() * 1000.0 / frames as f64;
    println!(
        "    {:<22} {mean_wait:>8.3} ms  (wait+copy per frame; loop mean {mean_loop:.3} ms incl. queue)",
        "wait+copy"
    );
}
