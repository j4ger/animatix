...
impl WorkspaceViewer<'_> {
    ...
    fn preview_ui(&mut self, ui: &mut egui::Ui) {
        ui.vertical(|ui| {
            ...
            // Click-to-select: only when no drag was active
            if response.clicked() && matches!(self.drag_state, DragState::None) {
                if let Some(click_pos) = response.interact_pointer_pos() {
                    let scene_point = screen_to_scene(click_pos);
                    let mut clicked_actor = false;
            ...
            if matches!(self.drag_state, DragState::None) {
                if let (Some(actor), Some(mouse)) = (self.selected_actor.as_ref(), pointer_pos) {
                    let scene = screen_to_scene(mouse);
                    let is_over_actor = self
                        .hit_regions
                        .iter()
                        .any(|(l, b)| l == actor && b.contains(scene));
                    if is_over_actor {
