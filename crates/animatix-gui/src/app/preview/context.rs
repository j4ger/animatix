...
impl PreviewContext<'_> {
    ...
    pub(crate) fn preview_screen_to_scene(&self, preview_rect: egui::Rect, screen: egui::Pos2) -> kurbo::Point {
        self.preview_transform(preview_rect).screen_to_scene(screen)
    }

    pub(crate) fn preview_scene_to_screen(&self, preview_rect: egui::Rect, scene: kurbo::Point) -> egui::Pos2 {
        self.preview_transform(preview_rect).scene_to_screen(scene)
    }

    pub(crate) fn handle_preview_selection(&mut self, ui: &mut egui::Ui, _preview_rect: egui::Rect, response: &egui::Response) {
