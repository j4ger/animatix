//! Icon glyph constants for use in UI.
//!
//! These are Phosphor Regular font glyph strings. They are defined here
//! (rather than via `egui_phosphor`) so the core crate does not depend on egui.
//!
//! If you add a new primitive with a new icon, add the glyph here and update
//! the primitive's `icon_id()` implementation.

/// Phosphor icon glyph for a square.
pub const SQUARE: &str = "\u{E45E}";
/// Phosphor icon glyph for a circle with a notch.
pub const CIRCLE_NOTCH: &str = "\u{EB44}";
/// Phosphor icon glyph for a minus sign.
pub const MINUS: &str = "\u{E32A}";
/// Phosphor icon glyph for a polygon.
pub const POLYGON: &str = "\u{E6D0}";
/// Phosphor icon glyph for a pen.
pub const PEN: &str = "\u{E3AA}";
/// Phosphor icon glyph for a right arrow.
pub const ARROW_RIGHT: &str = "\u{E06C}";
/// Phosphor icon glyph for a text "T".
pub const TEXT_T: &str = "\u{E48A}";
/// Phosphor icon glyph for a function symbol.
pub const FUNCTION: &str = "\u{EBE4}";
/// Phosphor icon glyph for code brackets.
pub const CODE: &str = "\u{E1BC}";
/// Phosphor icon glyph for an image.
pub const IMAGE: &str = "\u{E2CA}";
/// Phosphor icon glyph for a 3D vector.
pub const VECTOR_THREE: &str = "\u{EE62}";
/// Phosphor icon glyph for a bar chart.
pub const CHART_BAR: &str = "\u{E150}";
/// Phosphor icon glyph for a line chart trending up.
pub const CHART_LINE_UP: &str = "\u{E156}";
/// Phosphor icon glyph for a donut chart.
pub const CHART_DONUT: &str = "\u{EAA6}";
/// Phosphor icon glyph for cardinal arrows pointing outward.
pub const ARROWS_OUT_CARDINAL: &str = "\u{E0A4}";
/// Phosphor icon glyph for a gradient.
pub const GRADIENT: &str = "\u{EB42}";
/// Phosphor icon glyph for rows layout.
pub const ROWS: &str = "\u{E5A2}";
/// Phosphor icon glyph for columns layout.
pub const COLUMNS: &str = "\u{E546}";
/// Phosphor icon glyph for four squares grid.
pub const SQUARES_FOUR: &str = "\u{E464}";
/// Phosphor icon glyph for a stack.
pub const STACK: &str = "\u{E466}";
/// Phosphor icon glyph for a folder.
pub const FOLDER: &str = "\u{E24A}";
/// Phosphor icon glyph for an article.
pub const ARTICLE: &str = "\u{E0A8}";
/// Phosphor icon glyph for a happy mask.
pub const MASK_HAPPY: &str = "\u{E9F4}";
/// Phosphor icon glyph for filter / effects.
pub const FILTERS: &str = "\u{EB42}";
/// Phosphor icon glyph for a speaker with high volume.
pub const SPEAKER_HIGH: &str = "\u{E44A}";
