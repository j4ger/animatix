...
|----|-----|------------------------------|----------------|
| LG-1 | No series construction in expressions: no `sum`-style folder, no `factorial`, no `let` inside closures (parse error) | Taylor Sₙ(x) needs 7 hand-expanded terms gated by `step()`, with the degree knob inlined 6× into the closure body. ~~"no pure-fn calls from plot closures"~~ was wrong: verified working end-to-end 2026-09-06 (`plot_closure_calls_pure_fn_at_frame_time` — CallEnv resolves the UserFn through the frozen frame-env base on both paths) | `factorial`/`sum(list)` builtins registered in **both** eval paths (eval_shared + IR `BuiltinFn`); `sum_range(closure, lo, hi)` folder as a stdlib `NativeFn` (needs the caller env for closure invocation — `eval_builtin_fn` has no env param); closure-local `let` via `Expr::LetChain`. Design note: `docs/series_construction.md` |
