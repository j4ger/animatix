...
| PF-5 | P2 rebuild latency (font load, expand/typecheck, planner) | Partly done | System font DB is now loaded once process-wide and shared (commit `5b12b015`; `simple_build_only` 9.2ms→~366µs, `reactive_build_only`→~540µs). Remaining: large-scene `components_full`/`modules_full` (~15–17ms) expand/typecheck cost; gate `rebuild.*` |
