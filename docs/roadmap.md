...
**Refreshed 2026-09-05 (PF-6 rounds 6–8):** the round-5 blocker is resolved —
...
allocations; dynamic actors keep the fresh build).
...
Cumulative vs pre-PF-6: **−94.6% blocks, −98.0% bytes** (600 blocks /
500 KB → 32.2 / 9.9 KB per frame), and the `many_actors` bench pair is
reliable again (its ±9–17% "contradiction" was leak-driven swap pressure).
Gate-flag discipline held across all three rounds: every flag was
...
Remaining, ranked: (1) the per-frame evaluate loop is down to ~32 blocks —
the allocation lens now points at DHAT/peak-RSS on real scenarios
(GUI/export) rather than further micro-churn; `RenderCommand::local_bounds`
bezpath `bounding_box` (0.9%, needs a caching design) and `tracks`
BTreeMap→HashMap (iteration order is load-bearing: auto-colors, timeline
panels) stay scoped items; (2) PF-7 GPU/export throughput (orthogonal
second front). PF-2/PF-3 stay paused until the harness proves stable.
