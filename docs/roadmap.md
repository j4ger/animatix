...
- [x] **Phase 4: Interaction Layer Upgrade** — Added gesture primitives (`Gesture`/`GestureHandler`/`gesture_router.rs`) delegating to legacy drag handler. Extracted shared drag utilities (`drag_utils.rs`). Added keyboard shortcut registry (`interaction/keyboard.rs`) with declarative shortcuts replacing ad-hoc `handle_keyboard_shortcuts`. Added motion helper API (`components/anim.rs`) with `Transition`/`animate_toward`; migrated `animate_value_with_time` call sites.
