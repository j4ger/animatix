...
| 19 | **Unify coordinate system conventions** | `math_to_screen` in `plot.rs:1266` returns centered offsets (for plot-curve geometry), while `build/property.rs:240` returns absolute screen coords (offset + `at`). Same mathematical concept, different conventions. This causes confusion and is a footgun for plot rendering work. **Resolved:** Added `relative` parameter to `graph_math_to_screen` in `build/utils.rs` to explicitly control coordinate frame. |
| 20 | **`func` transitions as side-channel** | Pattern documented in `docs/architecture.md` (§10 Non-Interpolatable Property Transitions) and inline code comments on `AnimationTrack::func_transitions` and the `FuncTransition`/`FuncSource` types. No further action needed — the side-channel pattern is the correct architectural solution for property types that cannot implement `Interpolate`. |
