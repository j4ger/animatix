...
- [x] **Phase 1: Token Refoundation** — Replaced flat `design_tokens.rs` with 3-layer module system (`design_tokens/` dir with `primitive`, `semantic`, `spatial`, `typography`, `motion`, `util`). Migrated ~50 files from flat names (`BG_BASE`, `ACCENT_BLUE`) to semantic imports (`semantic::surface::BASE`, `semantic::accent::PRIMARY`). Some files retain `as` aliases (e.g., `use semantic::surface::WIDGET as BG_WIDGET`) for call-site compatibility — these resolve to the same semantic tokens at compile time. Deleted `PAD_*` duplicates (unified with `SPACE_*` scale). Updated `runtime.rs::install_theme` to consume semantic tokens.
