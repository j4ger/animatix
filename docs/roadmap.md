...
| PF-5 | P2 rebuild latency (font load, expand/typecheck, planner) | Partly done | System font DB is now loaded once process-wide and shared (commit `5b12b015`). Text/Code/Typst compilations are memoized process-wide keyed on all inputs (font-environment epoch guards staleness), and the build-time expression cache now keys on an O(1) environment stamp instead of hashing every env entry per evaluation. Combined effect on `text_rebuild/mixed_48_warm`: 49.6ms→9.5ms; all rebuild/full benches −23…−51% (`showcase_full` −51%, `build_reactive_timeline` −39%, `components_full`/`modules_full` ≈−30%). Remaining: residual ~9.5ms cached-build machinery (per-declaration full-env injection is still O(declarations²); incremental reuse across same-time declarations is the next lever); gate `rebuild.*` |
