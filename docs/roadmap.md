...
**Refreshed 2026-09-06 (PF-6 rounds 6–10 + rounds 11–12 + PF-7):** the round-5 blocker is resolved —
...
list is done (rounds 11–12), deferred with evidence (`tracks`
BTreeMap→HashMap — the original ≈30% string-map attribution covered five
map systems of which three are now gone; a storage refactor whose
iteration order is load-bearing for auto-colors/GUI panels needs fresh
`perf_driver` evidence showing `tracks.get` in the top-10 before it is
worth the risk), or closed as inherent (animated-text recompiles,
wgpu-internal bookkeeping). The GUI seek/scrub audit found and fixed the
hit-region/static-cache coupling; seek on static scenes now rides the
subtree cache. Remaining known costs, all measured and deliberate:
`simple_build_only` +3% (round-10 trade), `static_50_actors_with_items`
+9–12% (round-6 tooling-path trade), evaluate-loop residue ~32
blocks/frame (plot closures + wgpu internals), export readback wait
(overlapped by round 11).

PF-2/PF-3 stay paused until the harness proves stable; a fresh baseline
should be saved (`scripts/perf-bench.sh save`) before the next
optimization session — the 09-04 baselines predate rounds 6–12 and flag
improvements as drift.
