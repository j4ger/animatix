...
**Refreshed 2026-09-05 (PF-6 round 6):** the round-5 blocker is resolved —
...
Ledger row: **204.2 → 168.2 blocks (−17.6%) / 18,233 → 15,832 B (−13.2%)
per frame, perf_driver +5.1% (3× reproduced)**; gate wins `timeline_evaluate_*`
−50…−53%, `scrub_layout_scene_100frames` −47%. Of 7 gate flags, 5 failed
isolation (§4 contamination), `sample_all_tracks` +4.3% isolated (below
gate), `static_50_actors_with_items` +12–15% accepted (tooling-only path).
Cumulative vs pre-PF-6: **−72% blocks / −96.8% bytes**.

Full ledger (2026-09-04 → 2026-09-05):
...
| 6 | dense slot-id bounds table (round-5 prescription; `bounds_key_pool` deleted) | 168.2 | 15.8 KB | perf_driver +5.1%; `timeline_evaluate_*` −50…−53% in the gate |

Cumulative: **−72% blocks, −96.8% bytes**, and the `many_actors`
bench pair is reliable again (its ±9–17% "contradiction" was leak-driven
swap pressure). Gate-flag discipline held: this session's 7 flags — 5
failed isolation, 1 below gate, 1 accepted with a documented tooling-only
justification (see the PF-6 row).

Remaining, ranked: (1) env-key `format!` churn (~44 blocks / ≈1 KB/frame on
the 60-actor scenario — the largest measured allocation residue in the
evaluate loop) and `tracks` BTreeMap→HashMap (iteration order is
load-bearing: auto-colors, timeline panels; separate change);
`RenderCommand::local_bounds` bezpath `bounding_box` (0.9%, needs a caching
design) stays scoped behind these; (2) DHAT/peak-RSS on real scenarios
