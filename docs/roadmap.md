...
| Graph-hosted PlotCurve with a `stroke_progress` track draws nothing | Epicycles-style "wave grows" demos render a flat axis instead of the curve | `examples/gallery/epicycles.amx` (all scenes) | Plot stroke_progress reveal path — compare with gradient_descent's parametric trail |
