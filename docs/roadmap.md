...
**Refreshed 2026-09-05 (PF-6 rounds 6+7):** the round-5 blocker is resolved —
...
Round 7 (`a7b342c8`) then removed the top remaining allocation residue: one
shared key buffer across the frame-env injection chain, so the steady-state
pooled-env frame performs zero key allocations.
...
Cumulative vs pre-PF-6: **−83% blocks, −97.1% bytes** (600 blocks / 500 KB
→ 100.2 / 14.6 KB), and the `many_actors` bench pair is reliable again
(its ±9–17% "contradiction" was leak-driven swap pressure). Gate-flag
discipline held across both rounds: every flag was re-measured in
adjacent A/B isolation; 6 of 9 failed to reproduce (§4 contamination,
including two benches that measured change-*faster* in isolation), one is
below gate (`sample_all_tracks`, the per-track `Cell`), and
`static_50_actors_with_items` is accepted with a documented tooling-only
justification (see the PF-6 row).

Remaining, ranked: (1) the alloc-lens residue list in
`performance_evaluation.md` §3.5 is now dominated by inlined scene_eval
bodies (`evaluate_shape_render` command vectors ≈70 blocks/frame —
`RenderCommand` pooling/arena is the candidate) and `local_bounds`
bezpath `bounding_box` (0.9%, needs a caching design); (2) DHAT/peak-RSS
on real scenarios (GUI/export); (3) PF-7 GPU/export throughput
(orthogonal second front). PF-2/PF-3 stay paused until the harness proves
stable.
