...
|----|-----|------------------------------|----------------|
| ~~LG-1~~ | ~~No series construction in expressions~~ | **Done 2026-09-06** (design note `docs/series_construction.md`): `factorial`/`sum` fast-path builtins + `sum_range(closure, lo, hi)` folder (stdlib NativeFn) + block-bodied closures via `Expr::LetChain` (`{ let … }` after `=>`, newline-separated, `let`-token disambiguation). ~~"no pure-fn calls from plot closures"~~ was wrong: verified working end-to-end (`plot_closure_calls_pure_fn_at_frame_time`). Taylor-sin Sweep rewritten from 7 hand-expanded terms onto `sum_range` — renders identically at n=1/5/13. Limitation kept: no `return`/`for` statements inside closure blocks (pure fn bodies cover those) | — |
