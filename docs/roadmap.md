...
| PF-6 | P3 allocation / memory profile (peak RSS, per-frame clones) | Partly done 2026-09-04 | `alloc_driver` (dhat, `examples/`) captures steady-state per-frame allocations on the 60-actor dynamic scenario — deterministic, immune to the documented timing drift; first capture: 600 allocs / ≈500 KB churn per frame with the frame-env reserve as 86% of bytes (fixed, −81%); remaining ranked candidates (env-key strings, bezpath rebuilds, vector-path clones, layout residues, bounds-clone) in `performance_evaluation.md` §3.5. Remaining: DHAT/peak-RSS on real scenarios (GUI/export), per-frame `Vec`-clone hunt outside the driver scenario |
