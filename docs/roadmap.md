...
- [x] **Phase 4: Interaction Layer Upgrade (partial)** — Added gesture primitives (`Gesture`/`GestureHandler`/`gesture_router.rs`) delegating to legacy drag handler. Extracted shared drag utilities (`drag_utils.rs`). Added keyboard shortcut registry (`interaction/keyboard.rs`) with declarative shortcuts replacing ad-hoc `handle_keyboard_shortcuts`. Added motion helper API (`components/anim.rs`) with `Transition`/`animate_toward`; migrated `animate_value_with_time` call sites.
  - [ ] **Gesture handler extraction** — Extract focused handlers from `drag_handler.rs` (788 lines) into `gestures/` dir: move, scale, rotate, pivot, vertex, motion-path, reorder, marquee. Then delete legacy `drag_handler.rs`.
