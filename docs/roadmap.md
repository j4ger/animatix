...
| GPU `Filter` `blur`/color effects are not visibly applied in `animatix image` export | A `Filter` with `blur: 10` over a high-contrast checkerboard stays sharp. The `GpuFilterBackend` WGSL ping-pong blur path and the offscreen pending-composite blit look correct, and `GpuFilterBackend::new` succeeds; the smoke tests only assert `is_ok()` + size and never verify the content is actually blurred. Suspect a software-Vulkan (lavapipe) compute limitation OR a latent shader/pipeline bug (would also fail on real GPUs). | **Open — delegated to human review**: probe `dogfood/probes/009-filter-gpu-deferred` (repro + analysis + questions). Confirm on a real GPU/GUI whether blur works; if not, fix the shader/pipeline and add a content-level blur-verify regression test. Also consider surfacing the Filter scene-eval silent fallback as a diagnostic. |
| Typst math with implicit multi-letter coefficients fails to compile | `Typst, content: "$mc^2$"` / `"$E = mc^2$"` error with a generic "failed to compile Typst document", while `"$x^2 + y$"` works. Pre-existing Typst-integration parse issue (not caused by the 2026-08-27 text fixes). | **Open**: improve the error message to name the failing source, and investigate the Typst parse case (implicit multiplication / multi-char identifiers). Tracked with `mc^2` repro. |
