...
| Graph-hosted PlotCurve stroke color ignored | Declared `color:` renders as white; all curves in a Graph draw identically | `examples/projects/fft_explain.amx` (3 colored sines all white), `examples/gallery/epicycles.amx` | Plot path brush resolution (baked VelloPath fill/stroke) |
| Equation Fragment leading `+` renders as `1.` | Per-term Fragment highlights show `sin(x)1. sin(3x)/3...` instead of `+ sin(3x)/3` | `fft_explain.amx` Decomposition, `epicycles.amx` Hero | Equation/Fragment Typst join — `+` at fragment start parsed as enum marker |
