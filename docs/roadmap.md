...
| PF-5 | P2 rebuild latency (font load, expand/typecheck, planner) | Partly done | System font DB is now loaded once process-wide and shared (commit `5b12b015`; `simple_build_only` 9.2ms→~366µs, `reactive_build_only`→~540µs). Text/Code/Typst compilations are now memoized process-wide keyed on all inputs, so unchanged declarations skip the Typst engine across rebuilds (`text_rebuild/mixed_48_warm` 49.6ms→25.6ms; also fixes the cache key omitting `max_width`/`text_align`/`overflow` and removes a double compile in text assignments; staleness guarded by a font-environment epoch). Remaining: residual ~25ms cached-build machinery (env injection, `hash_value`, keyframe sorting) and large-scene `components_full`/`modules_full` expand/typecheck cost; gate `rebuild.*` |
