...
**Refreshed 2026-09-05 (PF-6 rounds 6–9):** the round-5 blocker is resolved —
...
buffer, which alone was 93% of the export frame's churn.
...
Remaining, ranked: (1) export-path churn — text/Typst
(`compile_text_fast`, `TextPath` clones, `interpolate_text_paths`;
~530 KB/frame on text-heavy `fft_explain`) and wgpu-internal bookkeeping;
(2) `RenderCommand::local_bounds` bezpath `bounding_box` (0.9%, needs a
caching design) and `tracks` BTreeMap→HashMap (iteration order is
load-bearing: auto-colors, timeline panels) stay scoped evaluate-lens
items; (3) PF-7 GPU/export throughput (orthogonal second front).
